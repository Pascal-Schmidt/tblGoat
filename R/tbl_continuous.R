...
continuous_tbl <- function(df, grouping_var = NULL, digit = 2,
  ...
  df %>%
    ...
    purrr::when(length(grouping_var) == 0 & base::nchar(modification) == 0 ~ .,
    ...
    dplyr::summarise_if(is.numeric, list(a_Mean = ~ paste0(round(mean(., na.rm = TRUE), digit), " (",
                                                           round(sd(., na.rm = TRUE), digit), ")"),
                                         b_Median = ~ paste0(round(median(., na.rm = TRUE), digit),  " (",
    ...
    dplyr::mutate(name = stringr::str_remove_all(name, ".*_[a-z]_(?!Mean)"),
    ...
    purrr::when(length(grouping_var) == 0 ~ dplyr::mutate(., Total = ifelse(!name %in% colnames(df),
                                                                            as.character(Total),
                                                                            "")) %>%
                  dplyr::select(-ID),
                # remove everything in variable row except variable itself
                ~dplyr::mutate_at(., vars(colnames(.)[-1]),
