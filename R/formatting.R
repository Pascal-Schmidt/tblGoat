...
formatting <- function(df, df_mode, grouping_var, header = TRUE, mode_tbl) {
  ...
  if(header & length(grouping_var) > 0) {
    ...
    df %>%
      ...
      { dplyr::bind_rows(mutate_all(., as.character),
      ...
      dplyr::mutate(group = ifelse(stringr::str_detect(group, "Total"), "Total", group),
  ...
  df_mode %>%
    ...
    purrr::when(mode_tbl == "numeric" ~ dplyr::mutate(., name = ifelse(stringr::str_detect(name, "Mean"),
                                                                      paste0(name, " (sd)"),
                                                                      name)) %>%
  ...
  dplyr::mutate(name = ifelse(stringr::str_detect(df_mode$name, "\\*"),
                              name,
                              paste0(strrep("&nbsp;", 3), name))) -> formatted_tbl

if(length(grouping_var) > 0) {

  formatted_tbl %>%
    purrr::when(header ~ purrr::set_names(., c("&nbsp;", table_names)),
                  ~ purrr::set_names(., c("&nbsp;", colnames(.)[-1]))) -> formatted_tbl
